...
export default function SalesReportView({ theme }: SalesReportViewProps) {
  ...
  // Sort by tanggal transaksi (terbaru ke terlama)
  const sortByTanggal = (data: SalesReport[]) => {
  ...
  const fetchSalesData = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setSalesData(sortByTanggal(data))
      } else {
        setSalesData([])
  ...
  const handleExportToExcel = () => {
    ...
    try {
      // Pastikan data sudah tersusun berdasarkan tanggal transaksi
      const sortedData = sortByTanggal(salesData)
      ...
      const exportData = sortedData.map((item, index) => {
        ...
        return {
          ...
          "TANGGAL INPUT": item.created_at
            ...
            : "-",
          // Kirim sebagai Date object agar Excel mengenali sebagai tanggal native
          "TANGGAL TRANSAKSI": item.tanggal ? new Date(item.tanggal) : "-",
          NAMA: item.spgNama,
          PRODUK: item.produk,
      ...
      // Format kolom TANGGAL TRANSAKSI (index 2, kolom C) sebagai tanggal DD/MM/YYYY
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={`px-4 py-3 text-sm text-right ${textSecondary}`}>
                            {isCurah ? (
                              <span className="font-semibold text-emerald-400">Rp {(item.hargaPcs || 0).toLocaleString("id-ID")}</span>
                            ) : (
                          ...
                          <td className="px-4 py-3 text-sm text-right">
                            {isCurah ? (
                              <span className="font-semibold text-emerald-400">{(item.penjualanGram || 0).toLocaleString()} gr</span>
                            ) : (
                              "-"
                            )}
                          </td>
